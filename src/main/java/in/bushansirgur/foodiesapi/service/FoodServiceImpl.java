...
public class FoodServiceImpl implements FoodService {
    ...
    private static final String DUMMY_IMAGE_URL = "https://images.unsplash.com/photo-1600891964599-f61ba0e24092?auto=format&fit=crop&w=800&q=80";
        ...
        return databaseEntries.stream()
                .map(food -> {
                    if (food.getImageUrl() == null || food.getImageUrl().isEmpty()) {
                        food.setImageUrl(DUMMY_IMAGE_URL);
                    }
    ...
    public FoodResponse readFood(String id) {
        ...
        if (existingFood.getImageUrl() == null || existingFood.getImageUrl().isEmpty()) {
            existingFood.setImageUrl(DUMMY_IMAGE_URL);
        }
    ...
    public void deleteFood(String id) {
        ...
        // Don't try to delete dummy image
        if (!imageUrl.equals(DUMMY_IMAGE_URL)) {
            String filename = imageUrl.substring(imageUrl.lastIndexOf("/") + 1);
            boolean isFileDelete = deleteFile(filename);
            if (isFileDelete) {
                foodRepository.deleteById(response.getId());
